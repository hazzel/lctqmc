...
for filename in list_of_files:
	...
	ax.plot( data[:,0], data[:,1], marker="o", color="b", markersize=10.0, linewidth=2.0, label=r"E")
	(_, caps, _) = ax.errorbar(data[:,0], data[:,1], yerr=data[:,2], marker='None', capsize=8, color="b", linewidth=0.0, )
	for cap in caps:
		cap.set_markeredgewidth(1.6)
		
	ax.plot( data[:,0], data[:,3], marker="o", color="g", markersize=10.0, linewidth=2.0, label=r"$R_{\text{cdw}}$")
	(_, caps, _) = ax.errorbar(data[:,0], data[:,3], yerr=data[:,4], marker='None', capsize=8, color="g", linewidth=0.0, )
	for cap in caps:
		cap.set_markeredgewidth(1.6)
	...
	if cnt == 0:
		...
		ax.set_xlabel(r"$1 / \Theta$", fontsize=18)
	ax.legend()
	cnt += 1
plt.tight_layout()
plt.show()
